...
MEMORY {
  ...
  tcb_entry        : ORIGIN = 0xa000, LENGTH = 0x0100
  tcb_att          : ORIGIN = 0xa100, LENGTH = 0x1F00
  tcb_body         : ORIGIN = 0xc000, LENGTH = 0x2000
...
SECTIONS
{
  ...
  .tcb_body   :
  {
    *(.tcb.body)
    ...
    __cur_offset = .;
    FILL(0x0343);
    . += 0x2000 - ( __cur_offset + 6 ) ;
    PROVIDE (__tcb_sendoff = .) ;
    *(.tcb.sendoff)
    . = ALIGN(2);
    PROVIDE (__tcb_allclear = .) ;
    *(.tcb.allclear)
    PROVIDE (__tcb_leave = .) ;
    *(.tcb.leave)
  }  > tcb_body
